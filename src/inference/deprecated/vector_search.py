...
class ProductSearch:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.region = config['vertex_ai']['region']
            self.index_id = config['vertex_ai']['index_id']  # Add this
            ...
            logging.error(f"Failed to initialize vector search: {str(e)}")
            raise
        
                # Add endpoint initialization
        self.index_endpoint = aiplatform.MatchingEngineIndexEndpoint(
            index_endpoint_name=f"projects/{self.project_id}/locations/{self.region}/indexEndpoints/{self.index_id}"
        )
    ...
    async def refresh_index(self):
        ...
        try:
            ...
            # Update query to match your BigQuery schema
            query = """
            SELECT 
                product_id,
                category_id,
            ...
            async for row in self.bq_client.query(query).result():
                ...
                embedding = model_server.model.get_product_embedding(dict(row))
